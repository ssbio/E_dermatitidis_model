...
$include "biomass_precursors.txt"
...
$include "reg_rxns_db.txt"

	medium(j)				reactions related to the definition of the growth medium
$include "medium.txt"
...
FILE RESULT /optfill_final_ED.txt/;
PUT RESULT;
RESULT.pw=1000;
...
find_met_obj..						met_obj =e= M * sum(i_m, x(i_m));
...
CPs_const_85(j_db)..				omega(j_db) =l= theta(j_db) + lambda(j_db);
CPs_const_86(j_db)..				rho(j_db) =l= theta(j_db) + lambda(j_db);
CPs_const_87(j_db)..				delta(j_db) =l= theta(j_db) + lambda(j_db);
CPs_const_88(j_db)..				delta(j_db) =g= v_2(j_db);
CPs_const_89(j_db)..				rho(j_db) =g= -v_2(j_db);
...
find_rxn_obj..	rxn_obj =e= M * sum(j_db, zeta(j_db));
...
find_omega_obj..	omega_obj =e= M * sum(j_db, omega(j_db));
...
PUT "Number of metabolites produced by raw model: ", temp/;
...
LOOP(p$(not done),
	...
	IF((CP1.modelstat eq 1),
		...
		PUT "INNER OBJECTIVE VALUE 2: ",temp//;
