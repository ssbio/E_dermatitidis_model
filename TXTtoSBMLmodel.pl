...
system("python convertWLSmodel_final.py");

#create the sbml file
open(SBMLFILE, ">iEde2091.xml") or die "could not create/write to sbml file, reason: $!\n";

open(MODELFILE, "<iEde2091.txt") or die "could not open/read the txt model file, reason: $!\n";
chomp(my @my_model = <MODELFILE>);
...
open(METSFILE, "<mets.txt") or die "could not open metabolites file, reason: $!\n";
...
open(RXNFILE, "<rxns.txt") or die "could not read the reaction list file, reason: $!\n";
...
open(STOICHFILE, "<Sij.txt") or die "could not read stoichiometric file, reason: $!\n";
...
#get reaction type
open(RXNTYPEFILE, "<rxntype.txt") or die "could not open reaction type file, reason: $!\n";
...
printf SBMLFILE "\t<model id=\"iEde2091\" name=\"iEde2091\" fbc:strict = \"true\">\n\n";
...
printf SBMLFILE "\t\t\t<parameter id=\"irr_f_ub\" value=\"10000\" constant=\"true\"/>\n";
printf SBMLFILE "\t\t\t<parameter id=\"irr_b_lb\" value=\"-10000\" constant=\"true\"/>\n";
printf SBMLFILE "\t\t\t<parameter id=\"irr_b_ub\" value=\"0\" constant=\"true\"/>\n";
printf SBMLFILE "\t\t\t<parameter id=\"rev_lb\" value=\"-10000\" constant=\"true\"/>\n";
printf SBMLFILE "\t\t\t<parameter id=\"rev_ub\" value=\"10000\" constant=\"true\"/>\n";
...
printf SBMLFILE "\t\t\t\t\t<fbc:fluxObjective fbc:reaction=\"biomass_si\" fbc:coefficient=\"1\"/>\n";
printf SBMLFILE "\t\t\t\t</fbc:listOfFluxObjectives>\n";
