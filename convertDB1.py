...
#*updated the output to write all files necessary for fba.gms (added exchange_reactions_1.txt'
#	irreversible_reactions_1.txt, and reversible_reactions_no_exchange such that this programs
#	output is the input to fba.gms.
#	This program is dependent on the nomenclature in kathKeys_1.txt, and was originally written
...
input1=open('nr3of4DB_reduced3_split_1.txt','r')

#these are the output file names, 'w' denotes write operation
output1=open('mets_db_1.txt','w')
output2=open('rxntype_db_1.txt','w')
output3=open('Sij_db_1.txt','w')
output4=open('rxns_db_1.txt','w')
output5=open('ex_rxns_db_1.txt','w')
output6=open('irrev_rxns_f_db_1.txt','w')
output9=open('irrev_rxns_b_db_1.txt','w')
output7=open('rev_rxns_no_ex_db_1.txt','w')
output8=open('reg_rxns_db_1.txt','w')
...
output1.write('/\n') #writing a '/' mark at the beginning of the output file 1, then new line (metabolites_1.txt)
output2.write('/\n') #writing a '/' mark at the beginning of the output file 2, then new line (rxntype_1.txt)
output3.write('/\n') #writing a '/' mark at the beginning of the output file 3, then new line (S_matrix_1.txt)
output4.write('/\n') #writing a '/' mark at the beginning of the output file 4, then new line (reactions_1.txt)
output5.write('/\n') #writing a '/' mark at the beginning of the output file 5, then new line (exchange_reactions_1.txt)
output6.write('/\n') #writing a '/' mark at the beginning of the output file 6, then new line (irreversible_reactions_f_1.txt)
output7.write('/\n') #writing a '/' mark at the beginning of the output file 7, then new line (reversible_reactions_no_exchange_1.txt)
output8.write('/\n') #writing a '/' mark at the beginning of the output file 8, then new line (regulated_reactions_1.txt)
output9.write('/\n') #writing a '/' mark at the beginning of the output file 8, then new line (regulated_reactions_b_1.txt)
...
for line in alllines1:	#for each line in the input file
	...
	if '->' in line and '<->' not in line:	#if a forward reaction	
		...
		if bool(re.search("biomass_ch",rxn)):
			objective=reactant
	...
	if rxn not in rxn_list:
		...
		output2.write( "'"+ rxn + "' " + state)	#writing the output in the rxntype_1.txt file
		output2.write('\n')	#newline
		output4.write("'"+ rxn + "'")
		output4.write('\n')	#newline
	...
	while k<len(dummy_list): #for every reactant, splitting the stochiometric coefficients
		...
		if react_compt not in met_list:
			...
			output1.write('\n')	#newline
		#joining the reactant name and rxn name and stoichiometric coefficinet to write into output3 (Sij_1.txt)
	...
	if product != '':
		...
		while kk<len(dummy_list2):  #for every product, splitting the stochiometric coefficients
			...
			# joining the product name and rsn name and stoichiometric coefficinet to write into output3 (Sij_1.txt)
